      ...
      <div className="relative" ref={dropdownRef}>
        <div
          className={`bg-white dark:bg-gray-800 border rounded-lg px-4 py-2.5 flex flex-wrap gap-2 min-h-[42px] cursor-pointer transition-all duration-300 ${
            error ? 'border-red-500 dark:border-red-500' : 'border-gray-300 dark:border-gray-600 focus-within:border-primary-500 dark:focus-within:border-primary-400'
          }`}
          onClick={() => setIsOpen(!isOpen)}
        >
          {selectedOptions.length === 0 ? (
            <span className="text-gray-400 dark:text-gray-500 py-0.5">Select options...</span>
          ) : (
            selectedOptions.map((option) => (
              ...
              >
                {option}
                <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 ml-1.5 cursor-pointer" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                filteredOptions.map((option) => (
                  <li
                    ...
                    className={`px-3 py-2 text-sm cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-200 ${
                      selectedOptions.includes(option) ? 'bg-primary-50 dark:bg-primary-900/10 text-primary-700 dark:text-primary-300' : ''
                    }`}
                    onClick={(e) => {
                      e.stopPropagation();
                      toggleOption(option);
